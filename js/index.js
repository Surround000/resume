~function(desW){
	var winW = document.documentElement.clientWidth;
	document.documentElement.style.fontSize = 100 * winW/desW + 'px';
}(400)

~function (){
	var step = 0, divList = null;
	var swp = new Swiper('.swiper-container',{
		loop:true,
		direction:'vertical',
		onSlidePrevEnd: function () {
            step--;
            change();
            if (step === 0) {
                step = 4;
            }
        },
        onSlideNextEnd: function () {
            step++;
            change();
            if (step === 5) {
                step = 1;
            }
        }
	})

	function change() {
        divList = document.querySelectorAll(".swiper-slide");
        [].forEach.call(divList, function (curDiv, index) {
            curDiv.id = index === step ? curDiv.getAttribute("trueId") : null;
        });
    }
}();

~function () {
    var audioBox = document.querySelector(".music"),
        myAudio = audioBox.getElementsByTagName("audio")[0];
        myAudio.src = "http://m1.maka.im/gfilemusic/93596.mp3"


    window.setTimeout(function () {
        myAudio.play();
        myAudio.addEventListener("canplay", function () {           
            audioBox.style.display = "block";
            audioBox.className += " musicMove";
        }, false);
    }, 1000);

    audioBox.addEventListener("click", function () {
        if (myAudio.paused) {
            myAudio.play();
            audioBox.className = "music musicMove";
            return;
        }
        myAudio.pause();
        audioBox.className = "music";
    }, false);
}();